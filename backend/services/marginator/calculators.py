...
class MarketplaceCalculator(BaseCalculator):
    def calculate_item(self, item: BaseItem, params: MarketplaceParams) -> CalculationResult:
        ...
        packaging = params.packaging_cost * item.quantity
        total_cost = item.cost_price * item.quantity

        tax = revenue * (params.tax_rate_percent / 100.0)
        # Чистая прибыль: выручка − себестоимость − комиссия − логистика − упаковка − налог
        net_profit = revenue - total_cost - commission - logistics - packaging - tax

        # ROI считаем от вложений (закупка + логистика + упаковка)
        investments = total_cost + logistics + packaging
