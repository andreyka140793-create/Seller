"""Document loading: xlsx, xls, csv, tsv, txt, docx, pdf, images."""
...
def is_supported(file_name: str) -> bool:
    ...
    return ext in (TABLE_EXTENSIONS | TEXT_EXTENSIONS | DOC_EXTENSIONS | PDF_EXTENSIONS | IMAGE_EXTENSIONS | {".doc"})
...
def load_document(file_bytes: bytes, file_name: str) -> LoadedDocument:
    ...
    if ext == ".docx":
        ...
        return LoadedDocument(kind="text", file_name=name, text=text)
    if ext == ".doc":
        raise RuntimeError(".doc is not supported. Save as .docx or .xlsx.")
    if ext == ".pdf":
    ...
    if ext in IMAGE_EXTENSIONS:
        mime = {".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp", ".bmp": "image/bmp", ".gif": "image/gif"}.get(ext, "image/jpeg")
        return LoadedDocument(kind="image", file_name=name, image_bytes=file_bytes, image_mime=mime)
...
def _read_excel_safe(file_bytes: bytes, file_name: str) -> pd.DataFrame:
    try:
        ...
        if "xlrd" in str(e).lower() or file_name.lower().endswith(".xls"):
            raise RuntimeError("Install xlrd>=2.0.1 or save as .xlsx") from e
        raise RuntimeError(f"Excel read error: {e}") from e


def _read_csv_flexible(file_bytes: bytes) -> pd.DataFrame:
    for sep in (";", ",", "\t", "|"):
        try:
            df = pd.read_csv(io.BytesIO(file_bytes), sep=sep, header=None, dtype=str, engine="python")
            if df.shape[1] >= 2:
                return df
        except Exception:
            continue
...
def _try_text_as_table(text: str) -> pd.DataFrame | None:
    ...
    for sep in ("\t", ";", ",", "|"):
        ...
        if len(widths) == 1 and list(widths)[0] >= 2:
            try:
                return pd.read_csv(io.StringIO("\n".join(lines)), sep=sep, header=None, dtype=str, engine="python")
            except Exception:
                continue
    return None
...
def _read_docx(file_bytes: bytes) -> str:
    try:
        ...
        raise RuntimeError("Install python-docx for .docx") from e
...
def _read_pdf(file_bytes: bytes) -> str:
    try:
        ...
        raise RuntimeError("Install pypdf for PDF") from e
    ...
    for page in reader.pages[:30]:
        ...
        if t.strip():
            parts.append(t)
    if not parts:
        raise RuntimeError("PDF has no text layer. Send a screenshot (JPG/PNG) instead.")
    return "\n".join(parts)
