...
class PriceUpload(Base):
    ...
    calc_mode = Column(String, default="marketplace") # marketplace или b2b
...
class AnalyzedItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    upload_id = Column(Integer, ForeignKey("price_uploads.id"))
    title = Column(Text, nullable=False)
    sku = Column(String, nullable=True)
    buy_price = Column(Float, nullable=False)        # Себестоимость
    est_sell_price = Column(Float, nullable=False)   # Выручка / Цена продажи
    net_profit = Column(Float, nullable=False)       # Чистая прибыль
    margin_pct = Column(Float, nullable=False)       # Маржинальность %
    roi_pct = Column(Float, nullable=False)          # ROI %
